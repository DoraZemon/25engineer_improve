...
//    }
    for (;;) {
        Fre_Arm = 1.f/DWT_GetDeltaT(&ArmControlCnt);
        g_arm.recover_dm();
        if(g_pc.check_pc_ctrl()){
            g_arm.update_control(g_rc.check_ready() && !g_pc.check_lost() , DWT_GetDeltaT(&ArmControlCnt));
        }else{
            g_arm.update_control(g_rc.check_ready() , DWT_GetDeltaT(&ArmControlCnt));
        }
