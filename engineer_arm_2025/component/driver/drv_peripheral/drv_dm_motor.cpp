    ...
    switch (motor_mode) {
        ...
        case DM_PV:
            can_device.init_rx(hcan,
                               ...
                               std::bind(&dm_motor_device::update_data, this, std::placeholders::_1),
                               rxSem);
            can_device.init_tx(hcan, 8, 0x100 + slaveId, tx_buff);
            break;
        case DM_VO:
            can_device.init_rx(hcan,
                               masterId,
                               std::bind(&dm_motor_device::update_data, this, std::placeholders::_1),
                               rxSem);
            can_device.init_tx(hcan, 8, 0x200 + slaveId, tx_buff);
            break;
        default:break;
    }
    type = type_;
...
void dm_motor_device::set_motor_enable() {
    ...
    can_device.tx_member.buf_data[2] = 0xff;
    can_device.tx_member.buf_data[3] = 0xff;
    can_device.tx_member.buf_data[4] = 0xff;
    can_device.tx_member.buf_data[5] = 0xff;
    can_device.tx_member.buf_data[6] = 0xff;
    can_device.tx_member.buf_data[7] = 0xfc;
    for (uint8_t tmp = 0; tmp < 10; tmp++) {
        if (can_device.send_msg() == osOK) {
            __NOP();
        }
        osDelay(3);
    }
...
void dm_motor_device::set_motor_disable() {
    ...
    can_device.tx_member.buf_data[5] = 0xff;
    can_device.tx_member.buf_data[6] = 0xff;
    can_device.tx_member.buf_data[7] = 0xfd;
    for (uint8_t tmp = 0; tmp < 10; tmp++) {
        if (can_device.send_msg() == osOK) {
            __NOP();
        }
...
void dm_motor_device::set_motor_save_zero_offset() {
    ...
    can_device.tx_member.buf_data[5] = 0xff;
    can_device.tx_member.buf_data[6] = 0xff;
    can_device.tx_member.buf_data[7] = 0xfe;
...
void dm_motor_device::set_motor_clear_error() {
    ...
    can_device.tx_member.buf_data[5] = 0xff;
    can_device.tx_member.buf_data[6] = 0xff;
    can_device.tx_member.buf_data[7] = 0xfb;
    for (uint8_t tmp = 0; tmp < 6; tmp++) {
        if (can_device.send_msg() == osOK) {
            __NOP();
        }
...
void dm_motor_device::set_ctrl_to_can_tx_buff() {
    ...
    switch (mode) {
        case DM_MIT://MIT模式
            ...
            can_device.tx_member.buf_data[0] = (pos_tmp >> 8);
            can_device.tx_member.buf_data[1] = pos_tmp;
            can_device.tx_member.buf_data[2] = (vel_tmp >> 4);
            can_device.tx_member.buf_data[3] = ((vel_tmp & 0xF) << 4) | (kp_tmp >> 8);
            can_device.tx_member.buf_data[4] = kp_tmp;
            can_device.tx_member.buf_data[5] = (kd_tmp >> 4);
            can_device.tx_member.buf_data[6] = ((kd_tmp & 0xF) << 4) | (tor_tmp >> 8);
            can_device.tx_member.buf_data[7] = tor_tmp;
            break;
        case DM_PV://位置速度模式
            pbuf = (uint8_t *) &ctrl_data.p_des;
            vbuf = (uint8_t *) &ctrl_data.v_des;
            can_device.tx_member.buf_data[0] = *pbuf;
            can_device.tx_member.buf_data[1] = *(pbuf + 1);
            can_device.tx_member.buf_data[2] = *(pbuf + 2);
            can_device.tx_member.buf_data[3] = *(pbuf + 3);
            can_device.tx_member.buf_data[4] = *vbuf;
            can_device.tx_member.buf_data[5] = *(vbuf + 1);
            can_device.tx_member.buf_data[6] = *(vbuf + 2);
            can_device.tx_member.buf_data[7] = *(vbuf + 3);
            break;
        case DM_VO://速度模式
            vbuf = (uint8_t *) &ctrl_data.v_des;
            can_device.tx_member.buf_data[0] = *vbuf;
            can_device.tx_member.buf_data[1] = *(vbuf + 1);
            can_device.tx_member.buf_data[2] = *(vbuf + 2);
            can_device.tx_member.buf_data[3] = *(vbuf + 3);
            break;
        default:break;
    }
}
...
void dm_motor_device::MIT_inter_set_motor_normalization_torque(float torque) {
    torque_set = torque + data.offset_current;
...
void dm_motor_device::set_stall_parameter(float _stall_current_max, float _stall_speed_max) {
    data.stall_speed_max = _stall_speed_max;
    data.stall_torque_max = _stall_current_max;
}
