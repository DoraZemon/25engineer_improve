...
void pump_device::update_data() {
    ...
    //todo 判断泵是否吸住
    judge_pump_holding(arm_pump_judge_holding, filtered_buff[0], data.pump_open_state.arm);
    judge_pump_holding(left_pump_judge_holding, filtered_buff[1], data.pump_open_state.left);
    judge_pump_holding(right_pump_judge_holding, filtered_buff[2], data.pump_open_state.right);
