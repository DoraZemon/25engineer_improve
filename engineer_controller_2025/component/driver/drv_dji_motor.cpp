...
void dji_motor_device::init(FDCAN_HandleTypeDef *_hcan,
                            bool reverse_flag,
                            uint32_t _id,
    ...
    if (this->type == DJI_M3508 || this->type == DJI_M2006) {
        if (_hcan == &hfdcan1) {
            if (_id <= 4) {
                this->fdcan_device.tx_member.buf_data = can1_tx_buff_0x200;
                this->fdcan_device.tx_member.id = 0x200;
            } else {
                this->fdcan_device.tx_member.buf_data = can1_tx_buff_0x1ff;
                this->fdcan_device.tx_member.id = 0x1FF;
            }
        } else if (_hcan == &hfdcan2) {
            if (_id <= 4) {
                this->fdcan_device.tx_member.buf_data = can2_tx_buff_0x200;
                this->fdcan_device.tx_member.id = 0x200;
            } else {
                this->fdcan_device.tx_member.buf_data = can2_tx_buff_0x1ff;
                this->fdcan_device.tx_member.id = 0x1FF;
            }
        }
        this->fdcan_device.tx_member.len = 8;
        this->fdcan_device.init_rx(_hcan,
                                 0x200 + this->id,
        ...
        if (_hcan == &hfdcan1) {
            if (_id <= 4) {
                ...
                this->fdcan_device.tx_member.buf_data = can1_tx_buff_0x2ff;
                this->fdcan_device.tx_member.id = 0x2ff;
            }
        } else if (_hcan == &hfdcan2) {
            if (_id <= 4) {
                this->fdcan_device.tx_member.buf_data = can2_tx_buff_0x1ff;
                ...
                this->fdcan_device.tx_member.buf_data = can2_tx_buff_0x2ff;
                this->fdcan_device.tx_member.id = 0x2ff;
            }
        }
        this->fdcan_device.tx_member.len = 8;
...
void dji_motor_device::set_current_to_can_tx_buff() const {
    ...
    this->fdcan_device.tx_member.buf_data[index - 1] = HIGH_BYTE(this->current_set);
    this->fdcan_device.tx_member.buf_data[index] = LOW_BYTE(this->current_set);
}
